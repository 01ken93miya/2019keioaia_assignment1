...
class Sonar_Data:
    ...
    def __init__(self, data_file_name='sonar_data.pkl'):
        
        self.index = -1
        with open ('%s' % (data_file_name), mode = 'rb') as f:
            sonar_data = pkl.load(f)
        self.samples = [(np.reshape(vector, vector.size), 1) for vector in self.standardize(sonar_data['train']['sonar'])] + [(np.reshape(vector, vector.size),0) for vector in self.standardize(sonar_data['train']['no_sonar'])]
    ...
    def main():
        ...
        data = Sonar_Data()
        model = Sonar_Model(activation=sigmoid)  # specify the necessary arguments
        trainer = Sonar_Trainer(data, model)
        costs, accuracies = trainer.train(0.001, 400) # experiment with learning rate and number of epochs
        model.save_model()
